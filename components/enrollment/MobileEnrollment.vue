  ...
  <div @click="viewDetails" class="registrar-card card-active">
    <div class="registrar-header">
      <div class="registrar-avatar">
        <img :src="selectedEnrollment.studentImage" alt="Registrar avatar" />
      </div>
      <div class="registrar-info">
        <div class="name-status-wrapper">
          <h3 class="registrar-name">{{ selectedEnrollment.studentName }}</h3>
          <div class="status-badge" :class="statusClass">
            <span class="status-dot"></span>
            {{ capitalizeFirst(selectedEnrollment.status) }}
          </div>
        </div>
        <p class="registrar-email">@{{ selectedEnrollment.studentId }}</p>
      </div>
      <div
        v-if="selectedEnrollment.status === 'pending'"
        ...
        <div class="dropdown">
          ...
          <transition name="dropdown">
            <div v-if="dropdownOpen" class="dropdown-menu" @click.stop>
              <button class="dropdown-item" @click="deactivate">
                <span class="dropdown-icon">
                  <MinusCircleIcon />
                </span>
                Reject
              </button>
              <button class="dropdown-item" @click="activate">
                <span class="dropdown-icon">
                  <PlusCircleIcon />
                </span>
                Approve
    ...
    <div class="registrar-stats">
      <div class="stat-group">
        ...
        <div class="stat-item">
          ...
          <div class="stat-value">{{ selectedEnrollment.courseCode }}</div>
        </div>
      </div>
      <div class="stat-group">
        <div class="stat-divider"></div>
        <div class="stat-item">
          ...
          <div class="stat-value">{{ selectedEnrollment.program }}</div>
        </div>
      </div>
    </div>
  </div>
...
const emit = defineEmits<{
  (e: "deactivate", value: Enrollment): void;
  (e: "activate", value: Enrollment): void;
  (e: "viewDetails"): void;
}>();
...
// Actions
const deactivate = () => {
  emit("deactivate", props.selectedEnrollment);
  dropdownOpen.value = false;
};
const viewDetails = () => {
  emit("viewDetails");
...
const activate = () => {
  emit("activate", props.selectedEnrollment);
  dropdownOpen.value = false;
};
...
const statusClass = computed(() => {
  switch (props.selectedEnrollment.status.toLowerCase()) {
    case "approved":
