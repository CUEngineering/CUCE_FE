  ...
  >
    <div class="page-header dashlet">
      <div class="title-and-filter">
        <h2 class="heading-txt">Recent Enrollments</h2>
      </div>
      <div
        style="margin: auto; margin-left: 5px"
        class="web profile-count pill p-grey pill-sm"
      >
        ({{ startRecord }} - {{ endRecord }}) of {{ totalRecords }}
      </div>
      <div v-if="enrollments.length > 0" class="search-and-actions">
        <Button variant="outline" size="sm" @click="goToEnrollments">
          View all
        </Button>
      </div>
    </div>
    ...
    <EmptyState
      ...
      description="You currently do not have any data in dashboard. this will be populated as actions are taken"
    >
      <template #icon>
        <img
...
const selectedEnrollmentCourse = computed(() =>
  selectedEnrollment.value
    ? getEnrollmentAsCourseListData(selectedEnrollment.value, enrollments.value)
    : undefined
);
